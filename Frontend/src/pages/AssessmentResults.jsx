...
const AssessmentResults = () => {
  ...
  const analysis = useMemo(
    () => analyzeAssessment(assessmentId),
    [assessmentId, analyzeAssessment],
  );
  ...
  const handleCloseAssessment = () => {
    if (
      confirm(
        "Are you sure you want to close this assessment? Students will no longer be able to take the test.",
      )
    ) {
    ...
    datasets: [
      {
        data: [
          ...
          assessment.results.filter((r) => r.score >= 60 && r.score < 80)
            .length,
          assessment.results.filter((r) => r.score >= 40 && r.score < 60)
            .length,
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="font-semibold text-gray-900">
                    Share Test Link with Students
                  </h3>
                </div>
                <p className="text-sm text-gray-600 mb-3">
                  Students can access the test using this link
                </p>
            ...
            <div className="bg-linear-to-br from-red-50 to-orange-50 rounded-xl md:rounded-2xl p-4 md:p-8 border border-red-200 mb-6 md:mb-8">
              <div className="flex flex-col md:flex-row items-start md:items-center gap-3 mb-4 md:mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm md:text-base text-gray-600">
                    Students scored below 60% on these topics - recommend
                    revisiting in class
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4 md:space-y-6">
                {analysis.weakConcepts.map((weakConcept, index) => {
                  const suggestions =
                    improvementSuggestions[weakConcept.topic] ||
                    improvementSuggestions["Teaching Methods"];
                      ...
                      <div className="flex flex-col md:flex-row items-start justify-between gap-3 md:gap-0 mb-4">
                        <div className="flex-1">
                          ...
                          <p className="text-xs md:text-sm text-gray-600">
                            {weakConcept.studentsAttempted} students attempted •{" "}
                            {weakConcept.percentage}% correct rate
                          </p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-2xl md:text-3xl font-bold text-red-600">
                            {weakConcept.percentage}%
                          </p>
                          <p className="text-xs md:text-sm text-gray-600">
                            Success Rate
                          </p>
            ...
            <div className="bg-linear-to-br from-green-50 to-emerald-50 rounded-xl md:rounded-2xl p-4 md:p-8 border border-green-200 mb-6 md:mb-8">
              <div className="flex items-center gap-3 mb-4 md:mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm md:text-base text-gray-600">
                    Students performed well on these topics (≥80%)
                  </p>
                </div>
              </div>
                ...
                {analysis.strongConcepts.map((concept, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 mb-2">
                      ...
                      <span className="text-green-600 font-bold text-base md:text-lg shrink-0">
                        {concept.percentage}%
                      </span>
                    </div>
                    <p className="text-sm text-gray-600">
                      {concept.studentsAttempted} students • {concept.correct}/
                      {concept.total} questions correct
                    </p>
                  </div>
                ))}
              </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="text-left py-2 md:py-3 px-2 md:px-4 text-xs md:text-sm font-semibold text-gray-600">
                        Student
                      </th>
                  ...
                  <tbody>
                    {assessment.results.map((result, idx) => {
                      ...
                      return (
                        ...
                        >
                          <td className="py-2 md:py-3 px-2 md:px-4 text-sm md:text-base text-gray-900 font-medium">
                            {result.studentName}
                          </td>
