...
const TimelineChart = ({ data = [] }) => {
  ...
  const getX = (i) =>
    PAD.l + (data.length === 1 ? cw / 2 : (i / (data.length - 1)) * cw);
  ...
  const linePoints = (arr) =>
    arr.map((v, i) => `${getX(i)},${getY(v)}`).join(" ");
      ...
      <svg
        ...
        {[0, 1, 2].map((i) => (
          <line
            ...
            x1={PAD.l}
            x2={W - PAD.r}
            y1={getY(min + (max - min) * (i / 2))}
            y2={getY(min + (max - min) * (i / 2))}
        ...
        <div
          ...
          <div className="flex justify-between gap-4">
            <span className="text-blue-500 font-semibold">
              You: {hoverData.val.score}
            </span>
            <span className="text-slate-500 font-semibold">
              Dept: {hoverData.val.deptAvg}
            </span>
          </div>
